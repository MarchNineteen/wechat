...
public enum WxCpErrorMsgEnum {
    ...
    CODE_42009(42009, "suite_access_token已过期；suite_access_token有时效性，需要重新获取一次"), /**
                                                                                 * 指定的userid未绑定微信或未关注微信插件；
                                                                                 * 需要成员使用微信登录企业微信或者关注微信插件才能获取openid
                                                                                 */
    CODE_43004(43004,
    ...
    CODE_301013(301013, "参数 telephone 不合法；telephone必须由1-32位的纯数字或’-‘号组成。"), /**
                                                                            * 参数 english_name
                                                                            * 不合法；参数如果有传递，不允许为空字符串，同时不能超过64字节，只能是由字母、数字、
                                                                            * 点(.)、减号(-)、空格或下划线(_)组成
                                                                            */
    CODE_301014(301014,
    ...
    CODE_302003(302003, "批量导入任务的文件中userid有重复"), /**
                                                 * 组织架构不合法（1不是一棵树，2 多个一样的partyid，3 partyid空，4 partyid name 空，5
                                                 * 同一个父节点下有两个子节点 部门名字一样 可能是以上情况，请一一排查）
                                                 */
    CODE_302004(302004,
